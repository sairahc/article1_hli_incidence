---
title: "linear_forestplot"
author: "Sairah L.F. Chen"
date: "06/11/2020"
output: html_document
---

```{r setup, include=FALSE}
library(ggplot2)
library(forestplot)
```



```{r forest plot ggplot, include=FALSE}
label <- c("Postmenopausal breast", 
           "Colorectal",
           "Lung",
           "Postmenopausal endometrial",
           "Postmenopausal ovarian",
           "Pancreatic",
           "Kidney")
estimate <- c(0.97,0.98,0.86,0.93,0.99,0.92,0.94)
lower_ci <- c(0.96,0.97,0.85,0.91,0.96,0.89,0.90)
upper_ci <- c(0.98,1.00,0.88,0.95,1.02,0.95,0.97)

df <- data.frame(label, estimate, lower_ci, upper_ci)

# reverses the factor level ordering for labels after coord_flip()
df$label <- factor(df$label, levels=rev(df$label))

fp <- ggplot(data=df, aes(x=label, y=estimate, ymin=lower_ci, ymax=upper_ci)) +
        geom_pointrange(color="blue") + 
        geom_errorbar(aes(ymin=lower_ci, ymax=upper_ci),width=0.1, color="blue")+
        theme(text = element_text(size=25))+
        geom_hline(yintercept=1, lty=2) +  # add a dotted line at x=1 after flip
        coord_flip() +# flip coordinates (puts labels on y axis)
        xlab("Cancer type") + ylab("HR (95% CI)")
        theme_bw()   # use a white background
    
labpos <- c(1.03)
labels <- c()
for (i in 1:7) {
  labels[i] = paste(sprintf("%.2f", estimate[i]), "(", sprintf("%.2f", lower_ci[i]), "-", sprintf("%.2f", upper_ci[i]), ")")
}
 # c("0.97(0.96-0.98)", "0.98(0.97-1.00", "0.86(0.85-")
        
fp_labels <- fp + geom_text(size=7, aes(y=labpos, label=labels, hjust=0)) 

# save to png
png("C:/Users/sch044/OneDrive - UiT Office 365/R/Paper1_new/output/forest_plot.png", width=1600, height=1200)
fp_labels+ylim(c(0.82,1.10))
dev.off()

fp_labels
```

